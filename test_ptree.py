...
class LeafTestCase(unittest.TestCase):
  ...
  def test_get_hash_should_calculate_hash_from_leaf_value(self):
    ...
    self.assertEqual("fcde2b2edba56bf408601fb721fe9b5c338d10ee429ea04fae5511b68fbf8fb9",
                      leaf.get_hash())
...
class ProofTreeTestCase(unittest.TestCase):
  ...
  def test_load_leaves_should_load_data_from_dicts(self):
    ...
    self.assertEqual(2, len(self.proof_tree.leaves))
  ...
  def test_init_document_should_set_empty_list(self):
    ...
    self.assertEqual(0, len(self.proof_tree.leaves))

  def test_init_document_should_initialize_leaf(self):
    data = {"foo": "bar"}
    self.proof_tree.init_document(data)
    self.assertEqual(1, len(self.proof_tree.leaves))
    leaf = self.proof_tree.leaves[0]
    self.assertEqual("foo", leaf.key)
    self.assertEqual("bar", leaf.value)
    self.assertEqual(0, leaf.position)
    self.assertEqual(None, leaf.hash_digest)
  ...
  def test_init_document_should_initialize_multiple_leaves_in_correct_order(self):
    ...
    self.assertEqual("a", leaf_one.key)
    self.assertEqual(0, leaf_one.position)
    leaf_two = self.proof_tree.leaves[1]
    self.assertEqual("foo", leaf_two.key)
    self.assertEqual(1, leaf_two.position)

  def test__get_root_should_return_none_if_no_leaves(self):
    root_hash = self.proof_tree._get_root([])
    self.assertEqual(None, root_hash)
  ...
  def test__get_root_should_return_root_from_one_leaf(self):
    ...
    self.assertEqual(leaf_hash, root_hash)
  ...
  def test__get_root_should_return_root_from_two_leaves(self):
    ...
    self.assertEqual("1bbfb30fc434390e7a7256694d995c2a756c32a3d716a40a24842164bf86ccb5", root_hash)
  ...
  def test__get_root_should_return_root_from_three_leaves(self):
    ...
    self.assertEqual("1a857260b79f9d209f201c635fb1cefae9e087af19a72642598eab112226b525", root_hash)
  ...
  def test__get_leaf_hashes_should_return_list_of_hashes(self):
    ...
    self.assertEqual([data[0]["hash"], data[1]["hash"]], self.proof_tree._get_leaf_hashes())

  def test_get_root_should_return_root_node_from_test_data(self):
    self.proof_tree.init_document(TEST_DATA)
    self.assertEqual("439518554dde3f594438a3fbcf028a24974e36153f345b98d74d803a82ecdd3c",
